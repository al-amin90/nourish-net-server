...
async function run() {
  try {
    ...
    const donatesCollections = client.db("nourishNetDB").collection("donate");
    ...
    app.get("/foods", async (req, res) => {
      ...
      if (sort) {
        options = {
          sort: { expiredDate: sort === "Ascending Order" ? 1 : -1 },
        };
      }
    ...
    // booking save a room data in db
    app.post("/donate", tokenVerify, async (req, res) => {
      const donatesData = req.body;
      const result = await donatesCollections.insertOne(donatesData);
      res.send(result);
    });

    app.get("/donate/:email", async (req, res) => {
      const email = req.params.email;
      const result = await donatesCollections.find({ email: email }).toArray();
      res.send(result);
    });
